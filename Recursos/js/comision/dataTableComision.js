 ...
 $(document).on("click", "#btn_eliminar", function() {
  let fila = $(this);        
    ...
    $(document).on("click", "#btn_Pdf", function () {
      ...
      let cantidadRegistros = tablaComision.rows().count();
    
      // Obtener el valor del filtro de búsqueda
      let buscar = $("#table-Comision_filter> label > input[type=search]").val().trim();
      ...
      if (cantidadRegistros > 0 && (buscar === '' || buscar !== '' && hayCoincidencias(buscar))) {
        window.open(
          "../../../TCPDF/examples/reporteriaComision.php?buscar=" + buscar,
          "_blank"
        );
      } else {
    ...
    function hayCoincidencias(buscar) {
      let datos = tablaComision.rows().data().toArray();
