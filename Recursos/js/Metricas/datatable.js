...
$(document).on("click", "#btn_Pdf", function () {
  ...
  let cantidadRegistros = tablaMetricas.rows().count();

  // Obtener el valor del filtro de búsqueda
  let buscar = $("#table-Metricas_filter> label > input[type=search]").val().trim();
  ...
  if (cantidadRegistros > 0 && (buscar === '' || buscar !== '' && hayCoincidencias(buscar))) {
    window.open(
      "../../../TCPDF/examples/reporteMetrica.php?buscar=" + buscar,
      "_blank"
    );
  } else {
...
function hayCoincidencias(buscar) {
  let datos = tablaMetricas.rows().data().toArray();
