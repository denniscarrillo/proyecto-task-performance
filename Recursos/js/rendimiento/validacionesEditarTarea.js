...
$(document).ready(function () {
  if (
    ...
    document.getElementById("tipoCliente").textContent != null
  ) {
    $tipoCliente = document.getElementById("tipoCliente").textContent;
  } else {
    $tipoCliente = $radioButton[1].checked
      ? $radioButton[1].value
      : $radioButton[0].value;
  }
...
let validarInputRTN = ($tipoCliente) => {
  ...
  estadoValidaciones.estadoME
    ...
    : "";
  estadoValidaciones.estadoMC && $tipoCliente != "Existente"
...
let validarInputNombreCliente = ($tipoCliente) => {
  ...
  estadoValidaciones.estadoCV = funciones.validarCampoVacio(
    inputsEditarTarea.nombre
  );
  estadoValidaciones.estadoCV && $tipoCliente != "Existente"
    ? (estadoValidaciones.estadoSL = funciones.validarSoloLetras(
        inputsEditarTarea.nombre,
        validaciones.soloLetras
      ))
    : "";
  estadoValidaciones.estadoSL && $tipoCliente != "Existente"
    ? (estadoValidaciones.estadoME = funciones.validarMasdeUnEspacio(
        inputsEditarTarea.nombre
      ))
    : "";
  estadoValidaciones.estadoME && $tipoCliente != "Existente"
    ? (estadoValidaciones.estadoMC = funciones.limiteMismoCaracter(
        inputsEditarTarea.nombre,
        validaciones.caracterMas3veces
      ))
    : "";
  estadoValidaciones.estadoMC && $tipoCliente != "Existente"
    ? funciones.caracteresMinimo(inputsEditarTarea.nombre, 13)
    : "";
};
...
let validarInputTelefono = () => {
  ...
  estadoValidaciones.estadoCV
    ...
    : "";
  estadoValidaciones.estadoSN
    ? (estadoValidaciones.estadoE = funciones.validarEspacios(
        inputsEditarTarea.telefono
      ))
    : "";
  estadoValidaciones.estadoE
    ? (estadoValidaciones.estadoMC = funciones.validarMismoNumeroConsecutivo(
        inputsEditarTarea.telefono,
        validaciones.caracterMas5veces
      ))
  ...
  estadoValidaciones.estadoMC
    ? funciones.caracteresMinimo(inputsEditarTarea.telefono, 8)
    : "";
};
...
let validarInputTitulo = () => {
  ...
  estadoValidaciones.estadoCV = funciones.validarCampoVacio(
    inputsEditarTarea.titulo
  );
  estadoValidaciones.estadoCV
    ? (estadoValidaciones.estadoSLN = funciones.validarSoloLetrasNumeros(
        inputsEditarTarea.titulo,
        validaciones.letrasNumeros
      ))
    : "";
  estadoValidaciones.estadoSLN
    ? (estadoValidaciones.estadoME = funciones.validarMasdeUnEspacio(
        inputsEditarTarea.titulo
      ))
    : "";
  estadoValidaciones.estadoME
    ? funciones.limiteMismoCaracter(
        inputsEditarTarea.titulo,
        validaciones.caracterMas3veces
      )
    : "";
